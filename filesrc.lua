...
spawn(function() 
    ...
    wait(1.5)
    require(game.ReplicatedStorage.Notification).new("<Color=Red>Roat Hub-V2 chào ae<Color=/>"):Display()
    wait(.5)
...
local Window = OrionLib:MakeWindow({Name = "Roat Hub-V2", HidePremium = false, IntroText = "Roat Hub-V2⚜️", SaveConfig = true, ConfigFolder = "NightHub"})
...
W:AddLabel("Roat Hub-V2 SCRIPT")
W:AddLabel("DiscordRoat Hub-V2")
local Section = W:AddSection({
    Name = "Status"
})
...
pcall(function()
    while wait() do
        if _G.AutoEliteHunter then
            if game:GetService("Players").LocalPlayer.PlayerGui.Main.Quest.Visible == true then
                OrionLib:MakeNotification({
                    Name = "Roat Hub-V2(Noitification)",
